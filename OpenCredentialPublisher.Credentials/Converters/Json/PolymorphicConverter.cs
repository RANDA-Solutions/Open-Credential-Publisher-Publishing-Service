        ...
        where TList : IList<TItem>, new()
    {
        ...
        public override TList Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            { // Start of 'wrapper' object
                ...
                if (KeyTypeLookup.TryGetValue(typeString, out var concreteItemType))
                {
                    var item = (TItem)JsonSerializer.Deserialize(jsonDoc.RootElement.GetRawText(), concreteItemType, options);
                    results.Add(item);
                }
                else if (!KeyTypeLookup.Any())
                {
                    var item = (TItem)JsonSerializer.Deserialize(jsonDoc.RootElement.GetRawText(), typeof(TItem), options);
                    results.Add(item);
                }
                else
                {
                    KeyTypeLookup.TryGetValue("*", out var catchAllType);
                    if (catchAllType != null)
                    {
                        var item = (TItem)JsonSerializer.Deserialize(jsonDoc.RootElement.GetRawText(), catchAllType, options);
                    }
                }
